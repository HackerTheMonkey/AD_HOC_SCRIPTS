create or replace procedure NSS_FRAUD_REPORT(operator_name In varchar2)
is
       file_handler_01 UTL_FILE.File_Type;-- define the file handler
       cursor FRAUD_REPORT(opr_name In varchar2) is -- the cursor that will contain the report query
              select a.caller||','||a.total_occupy_time||','||a.total_count_calls||','||a.trunkin||','||a.switch_code As report from temp1 a
               where
                 a.trunkin in (select b.trunk_no from bs_trunk_info b where b.trunk_type='Trunk IN' and b.trunk_manager=opr_name) and
                 a.switch_code in (select b.switch_code from bs_trunk_info b where b.trunk_type='Trunk IN' and b.trunk_manager=opr_name) and
       ...
       */
       FRAUD_REPORT_REC FRAUD_REPORT%rowtype;
...
Begin
      ...
      file_handler_01 := utl_file.fopen('TEST_DIR',operator_name||'_FRAUD_REPORT_'||To_char(CurrentDate,'yyyymmdd')||'.csv','A');
      UTL_FILE.put_line(file_handler_01,'caller,duration,no_of_calls,trunk_in,GT_Address');
      -- open the cursor
      Open FRAUD_REPORT(operator_name);
        Loop
        Exit When FRAUD_REPORT%Notfound; -- exit when there will be no more records to read from the cursor
             -- fetch data from the cursor
             Fetch FRAUD_REPORT Into FRAUD_REPORT_REC;
             -- write data to the file
             UTL_FILE.put_line(file_handler_01,FRAUD_REPORT_REC.REPORT);
        End Loop;
      -- close the cursor
      Close FRAUD_REPORT;
