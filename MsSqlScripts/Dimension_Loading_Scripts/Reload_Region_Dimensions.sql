...
with 
	...
	seventh_layer_levels(SeventhLevelId, SeventhLevelLongName, SeventhLevelParentId) as
	(
		select a.LevelId, LevelLongName, b.ParentLevelId from scrules..DimUserLevel a inner join scrules..DimUserLevelParent b
		on a.LevelId = b.LevelId where b.ParentLevelId in (select SixthLevelId from sixth_layer_levels)		
	)	
		
	select 
		a.FirstLevelLongName, b.SecondLevelLongName, c.ThirdLevelLongName, d.FourthLevelLongName, e.FifthhLevelLongName, f.SixthLevelLongName, g.SeventhLevelLongName
	from
	...
	full outer join seventh_layer_levels g
	on f.SixthLevelId = g.SeventhLevelParentId
	where
...
-- First way
select COUNT(1) from scrules..DimUserReportingAllHistory t where t.DimensionId = '7' and UserLongName like 'New Promotion Code' and Gen02LongName like 'Unmapped%';
-- Second Way
select COUNT(2) from scrules..DimUserFactInstance a inner join scrules..DimUserFactKey b on a.UserFactKey = b.UserFactKey where a.LongName like 'New%' and b.DimensionId='7';
...
into scrules..tmp_outlier_codes_bak_d7
from scrules..DimUserFactInstance a inner join scrules..DimUserFactKey b 
on a.UserFactKey = b.UserFactKey where a.LongName like 'New%' and b.DimensionId='7';
