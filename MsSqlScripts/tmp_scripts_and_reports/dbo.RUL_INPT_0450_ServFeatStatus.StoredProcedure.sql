USE [SCLoad]
GO
/****** Object:  StoredProcedure [dbo].[RUL_INPT_0450_ServFeatStatus]    Script Date: 10/18/2011 19:44:00 ******/
SET ANSI_NULLS ON
GO
...
-- HK20111128 Restricting pushing up the RevenueFlag from the feature level to the service level to subscribers in states A, S, and F
-- as per customer request.
-- HK20111110 Push the max service commitements from the feature level to the service level.
-- HK20111117 restrict pushing values from the feature level to the service level on RGU services
-- only
--=========================================================================================================================================
...
-- SH20110813: CR-014 Any customers with no RGUs will be cancelled. Create fail safe to exit if Level 02 RGU is not in DPRAH.
	...
	------------------------------------------------------------------------------------------------------------------------	
		...
		INNER JOIN	(
			...
			row_number() over (
			partition by datekey, billingsubsystemid, accountnumber, servicenumber 
					order by User11 desc 							
								 ,case when RIGHT(User09,1)= 6 THEN 10
								When RIGHT(User09,1)= 4 THEN 20
								When RIGHT(User09,1) = 3 THEN 30
								When RIGHT(User09,1) = 5 THEN 40
								When RIGHT(User09,1) = 1 THEN 50
								When RIGHT(User09,1) = 2 THEN 60
								When RIGHT(User09,1) = 7 THEN 70
								When RIGHT(User09,1) = 8 THEN 80
								     ELSE 100 END
